...
def login(username, password, session: requests.Session):
    ...
    def login(username, password, session: requests.Session):
        ...
        try:
            ...
            res = session.get(
                'http://43.155.99.203:30001/zfcaptchaLogin?type=resource&instanceId=zfcaptchaLogin&name=zfdun_captcha.js')
            # print(res.text)
            # print(res.text)
            rtk = re.findall('rtk:\'(.*)\'', res.text)[0]

            url = f'http://43.155.99.203:30001/zfcaptchaLogin?type=refresh&rtk={rtk}&time={nowTime}&instanceId=zfcaptchaLogin'
            a = json.loads(session.get(url).text)

            id = a['si']
            imtk = a['imtk']

            url = f'http://43.155.99.203:30001/zfcaptchaLogin?type=image&id={id}&imtk={imtk}&t={nowTime}&instanceId=zfcaptchaLogin'
            res = session.get(url)
            ...
            res = session.post('http://43.155.99.203:30001/zfcaptchaLogin', data=data)
            # print(res.text)
        ...
        res = session.get('http://43.155.99.203:30001/xtgl/login_getPublicKey.html?time=' + nowTime)
        ...
        url = f'http://43.155.99.203:30001/xtgl/login_slogin.html?time=' + nowTime
        res = session.get(url)
        doc = pq(res.text)
        ...
        res = session.post('http://43.155.99.203:30001/xtgl/login_slogin.html?time=' + nowTime, data=data)
        return res
    ...
    while True:
        ...
        elif '账号已锁定无法登录' in result.text:
            return {
                "msg": '密码错误，您密码输入错误已达规定次数，账号已锁定无法登录，次日自动解锁！如忘记密码，请与教务处联系!',
                "code": "702"
            }
        elif '密码错误' in result.text:
            return {
                "msg": '密码错误',
                "code": "703"
            }
